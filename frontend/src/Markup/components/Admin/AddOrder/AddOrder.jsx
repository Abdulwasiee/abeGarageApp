...
import { FaHandPointer } from "react-icons/fa"; // Importing the hand pointer icon
...
function AddOrder() {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await axios.get("http://localhost:2030/api/customers");
  ...
  const handleSelectCustomer = (customer_id) => {
    navigate(`/customers/customer-profile/${customer_id}?view=chooseVehicle`);
  };
