...
const CustomerTable = () => {
  ...
  const fetchCustomers = async (page) => {
    try {
      const response = await axios.get(
        `http://localhost:2031/api/customers/?page=${page}&limit=${itemsPerPage}`
      );

      console.log("Response Data:", response.data);

      // Assuming response.data contains `data` and `total` fields
  ...
  const handleView = (customer_id) => {
    navigate(`/customers/customer-profile/${customer_id}`);
  };
