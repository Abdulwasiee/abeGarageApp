...
function EditEmployeeList() {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        console.log("Fetching employee data for ID:", employee_id); // Debugging log

        const response = await axios.get(
          `http://localhost:2030/api/employees/${employee_id}`
        );
        ...
        setEmployeeData({
          ...
          active_employee: response.data.employee.active_employee === 1 ? 1 : 0,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/employees"); // Navigate back to the employees list after a short delay
      }, 2000);
    } catch (error) {
      setResponseMessage(
