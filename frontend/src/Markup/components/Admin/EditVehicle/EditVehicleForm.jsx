...
const EditVehicleForm = () => {
  ...
  useEffect(() => {
    const fetchVehicle = async () => {
      try {
        ...
        console.log("State after setting vehicle data:", vehicleData); // Check if state is updated
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate(`/vehicles/${vehicle_id}`); // Navigate back to the vehicles list after a short delay
      }, 1000);
    } catch (error) {
      setResponseMessage(
